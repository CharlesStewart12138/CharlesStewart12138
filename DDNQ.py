...
#重置噪声
    def reset_noise(self):
        # 权重的噪声
        epsilon_in_w = self._scale_noise(self.in_channels)
        epsilon_out_w = self._scale_noise(self.out_channels)
        epsilon_w = epsilon_out_w.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * epsilon_in_w.unsqueeze(0).unsqueeze(2).unsqueeze(3).unsqueeze(4)
        self.weight_epsilon.copy_(epsilon_w)
        # 偏置的噪声
        epsilon_out_b = self._scale_noise(self.out_channels)
        self.bias_epsilon.copy_(epsilon_out_b)
#调整噪声的水平
...
class Net(nn.Module):
    def __init__(self, n_states, n_actions):
        ...
        self.fc6 = NoisyLinear(16 * 5 * 5 * 5, n_actions)  # Assuming 5x5x5 output after convolutions
